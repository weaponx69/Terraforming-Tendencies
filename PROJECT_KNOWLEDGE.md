...
* **The Depletion Trigger:** A single micro-round ends organically when the automated drones mine **1/5th (20%) of the gatherable resources** located within the "Active Sector" (the sector of the most recently built Command Center).
* **Round Progression:** When the 20% threshold is reached, the game pauses (`Time.timeScale = 0`). The player automatically advances to the next generation, showing a summary screen with their performance/efficiency grade for that round. The player always moves to the next generation once the threshold is crossed, and resources are replenished.
