...
* **Command Post / Center Construction Unlock:** Fixed a progression lock during the expansion phase where the player was unable to build a new Command Post. The issue stemmed from `BlueprintDraftManager.knownBuildings` being cleared on reset and never registering default/starting buildings like the `"Command Post"`. Resolved this by automatically loading and registering all `BuildingSO` assets from `Resources` on reset, adding case/space-insensitive fuzzy matching and lazy-loading fallbacks, and explicitly unlocking the blueprint when transitioning to the expansion phase.
